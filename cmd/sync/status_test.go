...
func TestStatusCmd_Clean(t *testing.T) {
	...
	stdout, stderr, err := cmd.RunCmd(t, dir, "sync", "status")
	if err != nil {
...
func TestStatusCmd_JSONOutput(t *testing.T) {
	...
	stdout, stderr, err := cmd.RunCmd(t, dir, "sync", "status", "--format", "json")
	if err != nil {
		t.Fatalf("sync status --format json: %v\nstdout: %s\nstderr: %s", err, stdout, stderr)
	}
...
func TestStatusCmd_CheckServer_InSync(t *testing.T) {
	...
	stdout, stderr, err := cmd.RunCmd(t, dir, "sync", "status", "--check-server")
	if err != nil {
		t.Fatalf("sync status --check-server: %v\nstdout: %s\nstderr: %s", err, stdout, stderr)
	}
