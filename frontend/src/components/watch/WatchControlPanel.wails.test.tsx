...
vi.mock('../../config/runtime', () => ({ isWailsApp: true }))

const cliRuntimeStatus = vi.fn(async () => ({
  available: false,
  platform: 'darwin',
  installSupported: true,
  installHint: 'Installs the tld CLI via the official installer.',
}))

vi.mock('../../lib/desktop', () => ({
  getCLIRuntimeStatus: () => cliRuntimeStatus(),
  getWatchCommand: async (path: string) => `tld watch ${path}`,
  installCLI: vi.fn(async () => ({ available: true, platform: 'darwin', installSupported: true })),
}))
    ...
    watch: {
      ...
      settings: async () => ({
        ...
        settings: {
          ...
          thresholds: { max_elements_per_view: 100, max_connectors_per_view: 200, max_incoming_per_element: 20, max_outgoing_per_element: 20, max_expanded_connectors_per_group: 24 },
          visibility: { core_threshold_enabled: true, core_threshold: 1, tier_multiplier: 0.5, max_expansion_multiplier: 2, weights: { changed: 100, selected: 100, user_show: 100, user_hide: -100, high_signal_fact: 1.5, relationship_proximity: 1, dependency_fact: 0.2, utility_noise: -0.8, high_degree_noise: -1.5 } },
        ...
        embedding: { provider: 'local-lexical', model: 'x', dimension: 512 },
        overridden: false,
        descriptors: [{ key: 'watch.watcher', value: 'auto', description: '' }],
      }),
      scanProgress: async () => ({
        repository_id: 1,
...
describe('WatchControlPanel in Wails mode', () => {
  beforeEach(() => {
    cliRuntimeStatus.mockClear()
  })

  it('shows CLI install and copy-command controls instead of start', async () => {
    let tree: ReactTestRenderer | null = null
    await act(async () => {
      tree = create(<WatchControlPanel enabled />)
    })
    const text = collectText(tree!.root)
    expect(text).toContain('Install tld CLI')
